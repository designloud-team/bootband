	...
	<div id="main" class="col-sm-8 clearfix" role="main">
		...
		<article id="post-<?php the_ID(); ?>" <?php post_class('clearfix'); ?> role="article" itemscope itemtype="http://schema.org/BlogPosting">
			...
			<header>
				...
				<p class="meta"><?php _e("Posted", "bootband"); ?> <time datetime="<?php echo the_time('Y-m-j'); ?>" pubdate><?php the_time(); ?></time> <?php _e("by", "bootband"); ?> <?php the_author_posts_link(); ?>.</p>
		...
		<footer>
			...
			<?php the_tags('<p class="tags"><span class="tags-title">' . __("Tags","bootband") . ':</span> ', ' ', '</p>'); ?>
...
<div id="sidebar1" class="col col-lg-4 fluid-sidebar sidebar" role="complementary">
	...
	<div class="well">
		...
		<h3><?php _e("Image metadata","bootband"); ?></h3>
	...
	if ( $imgmeta['image_meta']['created_timestamp'] ) {
		echo __("Date Taken","bootband") . ": " . date("d-M-Y H:i:s", $imgmeta['image_meta']['created_timestamp'])."<br />"; }
		if ( $imgmeta['image_meta']['copyright'] ) {
			echo __("Copyright","bootband") . ": " . $imgmeta['image_meta']['copyright']."<br />"; }
			if ( $imgmeta['image_meta']['credit'] ) {
				echo __("Credit","bootband") . ": " . $imgmeta['image_meta']['credit']."<br />"; }
				if ( $imgmeta['image_meta']['title'] ) {
					echo __("Title","bootband") . ": " . $imgmeta['image_meta']['title']."<br />"; }
					if ( $imgmeta['image_meta']['caption'] ) {
						echo __("Caption","bootband") . ": " . $imgmeta['image_meta']['caption']."<br />"; }
						if ( $imgmeta['image_meta']['camera'] ) {
							echo __("Camera","bootband") . ": " . $imgmeta['image_meta']['camera']."<br />"; }
							if ( $imgmeta['image_meta']['focal_length'] ) {
								echo __("Focal Length","bootband") . ": " . $imgmeta['image_meta']['focal_length']."mm<br />"; }
								if ( $imgmeta['image_meta']['aperture'] ) {
									echo __("Aperture","bootband") . ": f/" . $imgmeta['image_meta']['aperture']."<br />"; }
									if ( $imgmeta['image_meta']['iso'] ) {
										echo __("ISO","bootband") . ": " . $imgmeta['image_meta']['iso']."<br />"; }
										if ( $pshutter ) {
											echo __("Shutter Speed","bootband") . ": " . $pshutter . "<br />"; }
											?>
										</div>
